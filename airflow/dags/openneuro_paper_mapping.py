...
def create_openneuro_paper_mapping_tables(**_context) -> None:
    ...
    CREATE TABLE IF NOT EXISTS openneuro_paper_citations (
        id SERIAL PRIMARY KEY,
        openneuro_id VARCHAR(255) NOT NULL,
        ...
        resolved_at TIMESTAMPTZ DEFAULT NOW(),
        run_id TEXT,
        UNIQUE (openneuro_id, primary_paper_doi, citing_paper_doi),
    ...
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS matched_primary_paper_doi TEXT;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS matched_primary_openalex_id TEXT;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS citation_source TEXT;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS citing_publication_date TEXT;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS citation_contexts JSONB;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS contexts_extracted_at TIMESTAMPTZ;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS citing_journal TEXT;
    ALTER TABLE openneuro_paper_citations ADD COLUMN IF NOT EXISTS citing_senior_author_country TEXT;

    CREATE INDEX IF NOT EXISTS idx_openneuro_paper_citations_dataset ON openneuro_paper_citations(openneuro_id);
    CREATE INDEX IF NOT EXISTS idx_openneuro_paper_citations_primary ON openneuro_paper_citations(primary_paper_doi);
    CREATE INDEX IF NOT EXISTS idx_openneuro_paper_citations_citing ON openneuro_paper_citations(citing_paper_doi);

    CREATE TABLE IF NOT EXISTS openneuro_paper_citation_classifications (
        id SERIAL PRIMARY KEY,
        openneuro_id VARCHAR(255) NOT NULL,
        primary_paper_doi TEXT NOT NULL,
        citing_paper_doi TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_openneuro_paper_citation_classifications_dataset
      ON openneuro_paper_citation_classifications(openneuro_id);
...
def fetch_and_persist_citations_batch(*, batch_index: int, dataset_ids: List[str], run_id: str, **context) -> Dict[str, Any]:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                SELECT
                    ...
                    p.publication_year
                FROM openneuro_dataset d
                JOIN openneuro_paper_map m ON m.openneuro_id = d.dataset_id
    ...
    citation_upsert = """
    INSERT INTO openneuro_paper_citations (
        openneuro_id, primary_paper_doi, citing_paper_doi,
    ...
    ON CONFLICT (openneuro_id, primary_paper_doi, citing_paper_doi) DO UPDATE SET
        matched_primary_paper_doi = COALESCE(EXCLUDED.matched_primary_paper_doi, openneuro_paper_citations.matched_primary_paper_doi),
        matched_primary_openalex_id = COALESCE(EXCLUDED.matched_primary_openalex_id, openneuro_paper_citations.matched_primary_openalex_id),
        citation_source = COALESCE(EXCLUDED.citation_source, openneuro_paper_citations.citation_source),
        citing_publication_date = COALESCE(EXCLUDED.citing_publication_date, openneuro_paper_citations.citing_publication_date),
        citing_journal = COALESCE(EXCLUDED.citing_journal, openneuro_paper_citations.citing_journal),
        citing_senior_author_country = COALESCE(EXCLUDED.citing_senior_author_country, openneuro_paper_citations.citing_senior_author_country),
        resolved_at = NOW(),
        run_id = EXCLUDED.run_id;
    """
        ...
        with conn.cursor() as cursor:
            ...
            if dataset_rows:
                logger.info("Citations batch %d: %d datasets, %d primary papers. %s", batch_index,
                            len({r["openneuro_id"] for r in dataset_rows}), len(dataset_rows),
                            format_budget(fetch_openalex_budget(session)))
            for rec_index, rec in enumerate(dataset_rows):
                openneuro_id = rec["openneuro_id"]
                primary_doi = normalize_doi(rec.get("paper_doi"))
                progress.update(rec_index, note=f"starting {openneuro_id} {primary_doi}", force=True)
                created_at = rec.get("created_at")
                if not primary_doi or not created_at:
                    continue
                seen_datasets.add(openneuro_id)
                ...
                for matched_primary_doi, matched_primary_openalex_id in dedup_versions:
                    ...
                    progress.update(note=f"{openneuro_id} {primary_doi}: {len(citing_papers)} citing papers from OpenAlex",
                                    force=True)
                    for citing_index, citing in enumerate(citing_papers, 1):
                        progress.update(note=f"{openneuro_id} {primary_doi}: citing paper {citing_index}/{len(citing_papers)}")
                        citing_doi = normalize_doi(citing.get("doi"))
                        if not citing_doi:
                            continue
                        edge_key = (openneuro_id, primary_doi, citing_doi)
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                WHERE c.openneuro_id = ANY(%s)
                ORDER BY c.openneuro_id, c.primary_paper_doi, c.citing_paper_doi;
                """,
                (dataset_ids,),
            )
            for row in cursor.fetchall():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            for rec_index, rec in enumerate(rows):
                progress.update(rec_index, note=f"{rec['openneuro_id']} <- {rec['citing_paper_doi']}")
                ...
                cursor.execute(
                    """
                    UPDATE openneuro_paper_citations
                    SET
