"""
Airflow DAG: Resolve CRCNS dataset -> paper DOI/title mappings.

Mirrors `openneuro_paper_mapping` but the resolver scrapes CRCNS landing/about
pages for DOIs (CRCNS has no metadata API).

Pipeline:
1. create_crcns_paper_mapping_tables  - DDL for crcns_paper_map et al.
2. fetch_unmapped_crcns_ids           - pick CRCNS datasets without mappings
3. build_batches                      - chunk for dynamic task mapping
4. resolve_and_persist_batch (mapped) - scrape /about, resolve via Crossref/OpenAlex, upsert
...
def create_crcns_paper_mapping_tables(**_context) -> None:
    ddl = """
    -- Ensure CRCNS table supports papers count
    ...
    -- Shared papers table (may already exist from DANDI/OpenNeuro mapping)
    ...
    CREATE TABLE IF NOT EXISTS crcns_paper_resolution_runs (
    ...
    CREATE TABLE IF NOT EXISTS crcns_paper_map (
        id SERIAL PRIMARY KEY,
        crcns_id VARCHAR(255) NOT NULL,
        crcns_title TEXT,
        ...
        run_id TEXT,
        UNIQUE (crcns_id, paper_doi),
        FOREIGN KEY (paper_doi) REFERENCES papers(paper_doi) ON DELETE CASCADE
    );

    CREATE INDEX IF NOT EXISTS idx_crcns_paper_map_crcns_id ON crcns_paper_map(crcns_id);
    CREATE INDEX IF NOT EXISTS idx_crcns_paper_map_paper_doi ON crcns_paper_map(paper_doi);
    ...
    CREATE TABLE IF NOT EXISTS crcns_paper_citations (
        id SERIAL PRIMARY KEY,
        crcns_id VARCHAR(255) NOT NULL,
        ...
        resolved_at TIMESTAMPTZ DEFAULT NOW(),
        run_id TEXT,
        UNIQUE (crcns_id, primary_paper_doi, citing_paper_doi),
    ...
    CREATE INDEX IF NOT EXISTS idx_crcns_paper_citations_dataset ON crcns_paper_citations(crcns_id);
    CREATE INDEX IF NOT EXISTS idx_crcns_paper_citations_primary ON crcns_paper_citations(primary_paper_doi);
    CREATE INDEX IF NOT EXISTS idx_crcns_paper_citations_citing ON crcns_paper_citations(citing_paper_doi);

    CREATE TABLE IF NOT EXISTS crcns_paper_citation_classifications (
        id SERIAL PRIMARY KEY,
        crcns_id VARCHAR(255) NOT NULL,
        primary_paper_doi TEXT NOT NULL,
        citing_paper_doi TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_crcns_paper_citation_classifications_dataset
      ON crcns_paper_citation_classifications(crcns_id);

    CREATE OR REPLACE VIEW crcns_dataset_papers AS
    SELECT
        d.dataset_id AS crcns_id,
        d.title AS crcns_title,
    ...
    LEFT JOIN crcns_paper_map m ON d.dataset_id = m.crcns_id
...
def fetch_unmapped_crcns_ids(**context) -> Dict[str, Any]:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO crcns_paper_resolution_runs (run_id, started_at, max_datasets_per_run, candidates_loaded, filtered_out, output_path, summary)
    ...
    logger.info(
        "Selected %d CRCNS dataset_ids (raw_loaded=%d filtered_out=%d max_cap=%s batch_size=%d include_already_mapped=%s)",
        len(dataset_ids),
        raw_count,
        filtered_out,
...
def build_batches(**context) -> List[Dict[str, Any]]:
    ti = context["ti"]
    payload: Dict[str, Any] = ti.xcom_pull(task_ids="fetch_unmapped_crcns_ids") or {}
...
def _persist_crcns_records(
    *,
    ...
    map_upsert = """
    INSERT INTO crcns_paper_map (crcns_id, crcns_title, paper_doi, doi_source, relation_type, resolved_at, run_id)
    VALUES (%s, %s, %s, %s, %s, NOW(), %s)
    ON CONFLICT (crcns_id, paper_doi) DO UPDATE SET
        crcns_title = COALESCE(EXCLUDED.crcns_title, crcns_paper_map.crcns_title),
        doi_source = COALESCE(EXCLUDED.doi_source, crcns_paper_map.doi_source),
        relation_type = COALESCE(EXCLUDED.relation_type, crcns_paper_map.relation_type),
        resolved_at = NOW(),
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if processed_datasets:
                ...
                cursor.execute(f"UPDATE crcns_dataset SET papers = 0 WHERE dataset_id IN ({placeholders});", ds_ids)
                cursor.execute(
                    f"""
                    UPDATE crcns_dataset d
                    SET papers = sub.cnt
                    FROM (
                        SELECT crcns_id, COUNT(*)::int AS cnt
                        FROM crcns_paper_map
                        WHERE crcns_id IN ({placeholders})
                        GROUP BY crcns_id
                    ) sub
                    WHERE d.dataset_id = sub.crcns_id;
                    """,
                    ds_ids,
                )
        conn.commit()
...
def fetch_and_persist_citations_batch(*, batch_index: int, dataset_ids: List[str], run_id: str, **context) -> Dict[str, Any]:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                SELECT
                    ...
                    p.publication_year
                FROM crcns_dataset d
                JOIN crcns_paper_map m ON m.crcns_id = d.dataset_id
    ...
    citation_upsert = """
    INSERT INTO crcns_paper_citations (
        crcns_id, primary_paper_doi, citing_paper_doi,
    ...
    ON CONFLICT (crcns_id, primary_paper_doi, citing_paper_doi) DO UPDATE SET
        matched_primary_paper_doi = COALESCE(EXCLUDED.matched_primary_paper_doi, crcns_paper_citations.matched_primary_paper_doi),
        matched_primary_openalex_id = COALESCE(EXCLUDED.matched_primary_openalex_id, crcns_paper_citations.matched_primary_openalex_id),
        citation_source = COALESCE(EXCLUDED.citation_source, crcns_paper_citations.citation_source),
        citing_publication_date = COALESCE(EXCLUDED.citing_publication_date, crcns_paper_citations.citing_publication_date),
        citing_journal = COALESCE(EXCLUDED.citing_journal, crcns_paper_citations.citing_journal),
        citing_senior_author_country = COALESCE(EXCLUDED.citing_senior_author_country, crcns_paper_citations.citing_senior_author_country),
        resolved_at = NOW(),
        run_id = EXCLUDED.run_id;
    """
            ...
            for rec in dataset_rows:
                ...
                for matched_primary_doi, matched_primary_openalex_id in dedup_versions:
                    ...
                    for citing in citing_papers:
                        ...
                        if not citing_doi:
                            continue
                        edge_key = (crcns_id, primary_doi, citing_doi)
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                WHERE c.crcns_id = ANY(%s)
                ORDER BY c.crcns_id, c.primary_paper_doi, c.citing_paper_doi;
                """,
                (dataset_ids,),
            )
            for row in cursor.fetchall():
    ...
    for i, ds_id in enumerate(dataset_ids, start=1):
        ...
        try:
            ...
            if not result.papers:
                unresolved.append({"crcns_id": str(ds_id), "crcns_title": title, "reason": result.reason or "no_papers_found", "error": result.error})
            else:
...
def summarize_run(**context) -> None:
    ti = context["ti"]
    seed: Dict[str, Any] = ti.xcom_pull(task_ids="fetch_unmapped_crcns_ids") or {}
    ...
    logger.info(
        "\n".join(
            [
                "CRCNS paper mapping run summary",
                f"- run_id: {run_id}",
...
dag = DAG(
    ...
    description="Resolve CRCNS dataset -> paper DOI/title mappings (scrapes /about pages)",
    schedule="@daily",
    catchup=False,
    tags=["datasets", "crcns", "papers", "mapping"],
...
create_tables_task = PythonOperator(
    task_id="create_crcns_paper_mapping_tables",
    python_callable=create_crcns_paper_mapping_tables,
    dag=dag,
)

fetch_ids_task = PythonOperator(
    task_id="fetch_unmapped_crcns_ids",
    python_callable=fetch_unmapped_crcns_ids,
    dag=dag,
)
