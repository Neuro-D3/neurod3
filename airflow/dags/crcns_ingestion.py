...
def register_crcns_source(**context):
    """Validate the produced crcns_dataset table against the dataset_table contract
    ...
    with get_db_connection() as conn:
        ...
        register_source(
            cursor,
            source_name="CRCNS",
            tables={"dataset_table": "crcns_dataset"},
        )
        conn.commit()


register_source_task = PythonOperator(
    task_id='register_crcns_source',
    python_callable=register_crcns_source,
    dag=dag,
)

create_crcns_table_task >> fetch_datasets_task >> enrich_datasets_task >> insert_datasets_task >> create_view_task >> verify_data_task >> register_source_task
