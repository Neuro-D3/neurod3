...
def enrich_dandi_descriptions_current_run(**context) -> List[Dict[str, Any]]:
    """
    ...
    logger.info("Starting description/modality enrichment for %d datasets", total)
    ...
    for idx, ds in enumerate(datasets, start=1):
        ...
        if idx == 1 or idx % 50 == 0 or idx == total:
            logger.info(
                "Enrichment progress: %d/%d (dataset_id=%s, version=%s)",
                idx,
                total,
                dataset_id,
                version,
...
def insert_dandi_datasets(**context):
    ...
    ti = context['ti']
    datasets = (
        ti.xcom_pull(task_ids='enrich_dandi_descriptions_current_run')
...
def create_unified_datasets_view(**context):
    ...
    try:
        ...
        logger.info(f"Successfully created/updated unified_datasets view ({row_count} rows)")
    except Exception as e:
...
enrich_descriptions_task = PythonOperator(
    task_id='enrich_dandi_descriptions_current_run',
    python_callable=enrich_dandi_descriptions_current_run,
    dag=dag,
)
...
create_dandi_table_task >> fetch_datasets_task >> enrich_descriptions_task >> insert_datasets_task >> create_view_task >> verify_data_task
